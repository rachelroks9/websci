...
var politicsBuffer = [0,0,0];
var typeBuffer = [];
var source = {};
var mapInfo = {};

var query = {track:'donald trump, hillary clinton, bernie sanders'};

client.stream('statuses/filter', query, function(stream) {
  stream.on('data', function(tweet) {      
    // chart one information
    // check if text includes name, then increase buffer
    if (tweet.text !== undefined) {
      if (tweet.text.match(/Donald Trump/i)) {
        politicsBuffer[0]++;
      }
      if (tweet.text.match(/Hillary Clinton/i)) {
        politicsBuffer[1]++;
      }
      if (tweet.text.match(/Bernie Sanders/i)) {
        politicsBuffer[2]++;
      }
    }
    
    // chart two information
    // look for source of tweet and add to an object
    var s = tweet.source;
    if (s !== null) {
      var regex = /(?=>).*(?=<)/;
      s = regex.exec(s);
    ...
    // chart three information
    // get the latitude and longitude of tweet as well as user and text
    if (tweet.place !== null && tweet.place !== undefined) {
      if (tweet.place.bounding_box !== null) {
        if (tweet.place.bounding_box.coordinates !== null) {
          var point = {"lat": tweet.place.bounding_box.coordinates[0][0],"lon": tweet.place.bounding_box.coordinates[0][1], "text": tweet.text};
...
// send data continuously to front end to bar graph
app.get('/overall',function (req, res) {
  res.json(politicsBuffer);
});

// send data continuously to front end to pie chart
app.get('/source',function (req, res) {
  res.json(source);
});

// send data continuously to front end to map chart
